...
// F2023 C1540-C1543: Check conditional argument against dummy data object
static void CheckConditionalArg(
  ...
  auto checkOneConsequent{
      [&](const evaluate::ActualArgument::ConditionalArg::Consequent &cons) {
        if (!cons) {
          return;
        }
        auto &consExpr{const_cast<evaluate::Expr<evaluate::SomeType> &>(
            cons->value())};
        ...
        if ((object.intent == common::Intent::Out ||
          ...
          messages.Say(
              ...
              object.intent == common::Intent::Out ? "OUT" : "IN OUT",
              dummyName);
        }
