  ...
  common::visit(
      common::visitors{
          ...
          [&](const parser::ConditionalArg &condArg) {
            ...
            using EvalTail = ActualArgument::ConditionalArg::ConditionalArgPartOrConsequent;

            // Analyze a single parser::Consequent into an evaluate Consequent
            auto analyzeConsequent{
                [&](const parser::ConditionalArg::Consequent &consequent)
                    -> EvalConsequent {
                  EvalConsequent result;
                  common::visit(
                      common::visitors{
                          [&](const common::Indirection<parser::Expr> &expr) {
                            if (MaybeExpr valExpr{
                                    context_.Analyze(expr.value())}) {
                              if (!valExpr->GetType()) {
                                context_.Say(
            ...
            auto analyzeCondArg{[&](const parser::ConditionalArg &ca,
              ...
              const auto &condition{
                  std::get<parser::ScalarLogicalExpr>(ca.t)};
              MaybeExpr condExpr{
              ...
              const auto &tailNode{
                  std::get<common::Indirection<
                      parser::ConditionalArgPartOrConsequent>>(ca.t)
                      .value()};
              ...
              common::visit(
                  common::visitors{
                      [&](const parser::ConditionalArg &innerCA) {
                        ...
                        if (innerResult) {
                          tail = common::CopyableIndirection<
                              EvalConditionalArg>{std::move(*innerResult)};
                        } else {
                          fatalErrors_ = true;
                        }
                      },
...
bool ArgumentAnalyzer::CheckConsequentTypesAndRanks(
  ...
  auto checkOne{
      [&](const ActualArgument::ConditionalArg::Consequent &cons) -> bool {
        if (!cons) {
        ...
        if (thisType) {
          ...
          if (refType->category() == TypeCategory::Derived &&
              !refType->IsUnlimitedPolymorphic() &&
              !refType->IsAssumedType()) {
  ...
  auto checkAll{[&](const ActualArgument::ConditionalArg &ca,
                    auto &self) -> bool {
