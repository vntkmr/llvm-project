...
subroutine test_conditional_arg
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'b'
  call sub((x > 0 ? a : b))
  ...
  ! TREE-NEXT: Scalar -> Logical -> Expr -> GT
  ! TREE: Consequent -> Expr -> Designator -> DataRef -> Name = 'a'
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> ConditionalArg
  ! TREE-NEXT: Scalar -> Logical -> Expr -> GT
  ! TREE: Consequent -> Expr -> Designator -> DataRef -> Name = 'b'
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'c'
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Nil
  call sub((flag ? a : .NIL.))
  ...
  ! TREE: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Multiply
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'r2'
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'x'
  ...
  ! TREE: ConditionalArgPartOrConsequent -> Consequent -> Expr -> FunctionReference -> Call
  ! TREE-NEXT: ProcedureDesignator -> Name = 'arr'
  ...
  ! TREE: ActualArg -> ConditionalArg
  ! TREE: Consequent -> Expr -> FunctionReference -> Call
  ! TREE: ConditionalArgPartOrConsequent -> Consequent -> Nil
  call sub((flag ? arr(3) : .NIL.))

  ! Test 15: Multiple conditional args in one call
  ! CHECK: CALL sub(( flag ? a : b ), ( flag2 ? c : x ))
  ! TREE: ActualArg -> ConditionalArg
  ! TREE: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'b'
  ! TREE: ActualArg -> ConditionalArg
  ! TREE: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'x'
  ...
  ! TREE: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Parentheses -> Expr -> Subtract
  ...
  ! TREE-NEXT: Scalar -> Logical -> Expr -> Designator -> DataRef -> Name = 'flag2'
  ! TREE-NEXT: Consequent -> Nil
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'a'
...
subroutine test_typeless_consequents
  ...
  ! Test 29: BOZ literal (hex) as consequent
  ! CHECK: CALL sub(( flag ? a : z"ff" ))
  ! TREE: ActualArg -> ConditionalArg
  ! TREE: Consequent -> Expr -> Designator -> DataRef -> Name = 'a'
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> LiteralConstant -> BOZLiteralConstant
  call sub((flag ? a : z'FF'))

  ! Test 30: BOZ literal (binary) as consequent
  ...
  ! Test 31: BOZ literal (octal) as consequent
  ...
  ! Test 33: BOZ in middle branch of multi-branch conditional
  ...
  ! Test 34: NULL() as consequent
  ! CHECK: CALL sub(( flag ? a : null() ))
  ! TREE: ActualArg -> ConditionalArg
  ! TREE: Consequent -> Expr -> Designator -> DataRef -> Name = 'a'
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> FunctionReference -> Call
  ! TREE: ProcedureDesignator -> Name = 'null'
  call sub((flag ? a : null()))

  ! Test 35: NULL() as first consequent
  ! CHECK: CALL sub(( flag ? null() : a ))
  ! TREE: ActualArg -> ConditionalArg
  ! TREE: Consequent -> Expr -> FunctionReference -> Call
  ! TREE: ProcedureDesignator -> Name = 'null'
  ! TREE: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'a'
  call sub((flag ? null() : a))

  ! Test 36: NULL() in both branches
...
contains
  subroutine mod_sub_with_condarg(flag, a, b)
    ...
    ! Test 37: Conditional arg inside a module procedure
  ...
  subroutine mod_sub_with_nil(flag, a)
    logical, intent(in) :: flag
    integer, intent(in) :: a
    ! Test 38: .NIL. conditional arg inside a module procedure
  ...
  subroutine mod_sub_with_boz(flag, a)
    logical, intent(in) :: flag
    integer, intent(in) :: a
    ! Test 39: BOZ conditional arg inside a module procedure
  ...
  subroutine mod_sub_with_null(flag, a)
    logical, intent(in) :: flag
    integer, intent(in) :: a
    ! Test 40: NULL() conditional arg inside a module procedure
...
subroutine test_use_module_condarg
  ...
  ! Test 41: Conditional arg using module procedure
  ...
  ! Test 42: Conditional arg at call site using procedures from USEd module
  ! CHECK: CALL ext(( cond ? x : y ))
  ! TREE: ActualArg -> ConditionalArg
  ! TREE: Consequent -> Expr -> Designator -> DataRef -> Name = 'x'
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'y'
  call ext((cond ? x : y))

  ! Test 43: BOZ conditional arg at call site after USE
  ...
  ! Test 44: NULL() conditional arg at call site after USE
  ...
  ! Test 45: Multi-branch conditional arg at call site after USE
...
subroutine test_kinds
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'i4b'
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'i8b'
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'r4b'
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 'r8b'
...
subroutine test_character
  ...
  ! TREE-NEXT: ConditionalArgPartOrConsequent -> Consequent -> Expr -> Designator -> DataRef -> Name = 's2'
  call sub((flag ? s1 : s2))
